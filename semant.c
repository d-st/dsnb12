...
Type *checkCallStm(Absyn * node, Table * symtab)
{
  ...
  while (p && !p->isEmpty) {
    if (p->isRef && arg->u.expList.head->type != ABSYN_VAREXP) {
      error("procedure '%s' argument %d must be a variable in line %d",
            symToString(node->u.callStm.name), n, node->line);
    }
    arg = arg->u.expList.tail;
    p = p->next;
...
Type *checkProcDec(Absyn * node, Table * symtab)
{
  ...
  while (!(tmp->u.decList.isEmpty)) {
    ...
    entry =
	newVarEntry(checkNode(tmp->u.decList.head->u.varDec.ty, symtab),
		    FALSE);
...
Type *checkNode(Absyn * node, Table * symtab)
{
  ...
  case ABSYN_ARRAYTY:
    ...
    break;
  case ABSYN_TYPEDEC:
    /* return checkTypeDec(node, symtab); */ // already done in 1st run
    break;
  case ABSYN_PROCDEC:
...
void enter_proc_decs(Absyn * program, Table * symtab)
{
  ...
  while (!program->u.decList.isEmpty) {
	...
	while (!(tmp->u.decList.isEmpty)) {
	  p_new = newParamTypes(checkNode(tmp->u.decList.head, symtab),
				tmp->u.decList.head->u.parDec.isRef,
				NULL);
	  if (paramTypes == NULL)
	    paramTypes = p_new;
	  else
...
Table *check(Absyn * program, boolean showSymbolTables)
{
  ...
  Entry *builtinProc_time =
      newProcEntry(newParamTypes(builtinType_int, TRUE, emptyParamTypes()),
